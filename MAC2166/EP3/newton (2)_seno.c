...
int main()
{
    ...
    for (r = 0; r <= N; r++) {
        for (s = 0; s <= N; s++) {
            x_real = l + r*delta;
            x_imag = l + s*delta;
            if(TESTE) fprintf(teste, "x = (%.10g + %.10gi)\n", x_real, x_imag);
            check = newton (&x_real, &x_imag, max_iter, eps);
            if(TESTE) fprintf(teste, "%d\n", check);
            if (check == 0) fprintf(saida, "N ");
            else fprintf(saida, "%.10g %.10g ", x_real, x_imag);
            if (TESTE) {
                if (!check) fprintf(teste, "N\n");
                else fprintf(teste, "(%.10g + %.10gi)\n\n ", x_real, x_imag);
            }
        }
        fprintf(saida, "\n");
    }
...
int newton(double *x_real, double *x_imag, int max_iter, double eps)
{
    ...
    for (i = 0; i < max_iter; i++) {
        calcula(*x_real, *x_imag, &p_real, &p_imag, &d_real, &d_imag);
        if (TESTE) {
            fprintf(teste, "p = (%.10g + %.10gi)\n", p_real, p_imag);
            fprintf(teste, "d = (%.10g + %.10gi)\n", d_real, d_imag);
        }
